...
@app.route("/add_slack_recipient", methods=["POST"])
@admin_required
def add_slack_recipient():
    slack_user_id = request.form.get("slack_user_id", "").strip()
    name = request.form.get("name", "").strip()
    if not slack_user_id or not name:
        return "Missing Slack User ID or name", 400
    if len(slack_user_id) > 50 or len(name) > 80:
        return "Input too long", 400
    if SlackNotificationRecipient.query.filter_by(slack_user_id=slack_user_id).first():
        return "Recipient already exists", 400
    ...
    log_event(f"Added Slack notification recipient: {name}")
    db.session.commit()
    return redirect(url_for("admin"))

@app.route("/toggle_slack_recipient/<int:recipient_id>", methods=["POST"])
@admin_required
def toggle_slack_recipient(recipient_id):
    recipient = SlackNotificationRecipient.query.get(recipient_id)
    if recipient:
        recipient.is_active = not recipient.is_active
        status = "activated" if recipient.is_active else "deactivated"
        log_event(f"{status.capitalize()} Slack notification recipient: {recipient.name}")
        db.session.commit()
    return redirect(url_for("admin"))

@app.route("/delete_slack_recipient/<int:recipient_id>", methods=["POST"])
@admin_required
def delete_slack_recipient(recipient_id):
    recipient = SlackNotificationRecipient.query.get(recipient_id)
    if recipient:
        log_event(f"Deleted Slack notification recipient: {recipient.name}")
